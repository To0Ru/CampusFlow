...
| 地址 | 期望 |
|---|---|
| `http://www.baidu.com` | 200 且正文含 `baidu`，**或**跳到 `*.baidu.com` |

### 为什么不能只看状态码

百度对**浏览器 UA** 会做 HTTP→HTTPS 强制升级：

```
UA = curl/8.x        ->  200  (直出)
UA = Chrome/...      ->  302  Location: https://www.baidu.com/
```

拿 curl 测很容易得出“百度返回 200”这个**错误结论**（本项目的开发过程里就踩了一次）。
程序里用的是浏览器 UA，所以拿到的是 302。

如果简单地把“非 200 就当断网”，就会把升级跳转误判成门户劫持，
于是**每次探测都以为断网**，然后不停地注销 + 重连（真实发生过的坑）。

### 正确的判据：看 Location 指向哪

保持 `redirects(0)` 不跟跳转，然后自己判断跳转目标：

| Location | 判定 |
|---|---|
| `https://www.baidu.com/` | 自己人，升级跳转 → **正常** |
| `http://10.102.250.36/#/login?...` | 门户 | → **被劫持** |

实现里用 `ends_with(".baidu.com")` 而不是 `ends_with("baidu.com")`，
避免 `notbaidu.com` 这种域名混进来。相对跳转视为同域，放行。

> 门户劫持时也有可能直接返回 `200` + 登录页（而不是 302），
> 所以**正文校验不能省**。

### 关于单点探测

目前只用一个点（百度），足够用。但单点的风险是：百度自己抽风时会被误判为断网，
进而触发一次不必要的注销 + 重连。想加备用点，在 `net.rs` 的 `PROBES` 里追一条即可，格式一致。
